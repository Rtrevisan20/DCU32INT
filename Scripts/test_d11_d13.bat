...
for %%F in ("%TEMP_D11%\*.dcu") do (
    echo Testing %%~nF...
    "%EXE%" "%%F" -U* -AC -I 2>&1 | findstr "Total\|Error\|Warning at" >> d11_results.txt
)
...
for %%F in ("%TEMP_D13%\*.dcu") do (
    echo Testing %%~nF...
    "%EXE%" "%%F" -U* -AC -I 2>&1 | findstr "Total\|Error\|Warning at" >> d13_results.txt
)
...
echo.
echo Sample errors (D11):
findstr "Error" d11_results.txt | head -20
echo.
echo Sample errors (D13):
findstr "Error" d13_results.txt | head -20
